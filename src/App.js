...
function App() {
  const [search, setSearch] = useState('denver');
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${search}&appid=76d6e5af79a3625c2afe90aab1cc5be`)
      if (componentMounted) {
      ...
      <div className="card">
        <div className="search">
          <form onSubmit={handleSubmit}>
            <input type="text" className="search-bar" placeholder="Search" name="search" value={input}
        ...
        <div className="weather loading">
          ...
          <h1 className="temp">{tempinC} °C</h1>
          <div className="flex">
